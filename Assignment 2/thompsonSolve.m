...
function ts = thompsonSolve(n,d,epsilon,max_iterations,seed)
    ...
    x0 = [x0, transpose(x0_lastcol)]; % add final column to xo
    ...
    while repeat == true
        ...
        % f calculations:
        f = func(x); % Calculate f
        g_f = grad_f(x); % Calculate grad of f
        
        % Numerical calculation of the Hessian of f
        bump = epsilon; % assign bump value
        H_f = zeros(n_bar); 
        l = 0;
        for i1 = 1:n
            for i2 = 1:d
                for j1 = 1:n
                    for j2 = 1:d
                        ...
                        bump_xij(i1,i2) = bump_xij(i1,i2) + bump;
                        bump_xij(j1,j2) = bump_xij(j1,j2) + bump;
                        bump_xi(i1,i2) = bump_xi(i1,i2) + bump;
                        bump_xj(j1,j2) = bump_xj(j1,j2) + bump;
                        f_bump_xij = func(bump_xij);
                        f_bump_xi = func(bump_xi);
                        f_bump_xj = func(bump_xj);
                        f_init = func(x);    
                        H_f(l) = (f_bump_xij - f_bump_xi - f_bump_xj + f_init)/bump^2;
                    end
                end
            end
        end
